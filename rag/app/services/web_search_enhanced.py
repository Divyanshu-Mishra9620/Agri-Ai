...
class EnhancedWebSearch:
    ...
    async def search_agricultural_data(
        ...
        if results["sources"]:
            ...
            results["recommendations"] = self._extract_recommendations(
                results["sources"]
            )

        self.search_cache[cache_key] = {"data": results, "timestamp": datetime.now()}
    ...
    def _build_search_queries(
        self, query: str, category: str = "general", region: str = None
    ) -> List[str]:
...
async def main():
    ...
    results = await search.search_agricultural_data(
        query="rice leaf blast", category="disease", region="Punjab"
    )
