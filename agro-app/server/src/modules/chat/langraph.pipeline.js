...
async function analyzeContext(state) {
  ...
  console.log("🔍 [DEBUG] Config check - GROQ key exists:", !!config.groqApiKey);
...
Respond in JSON format:
{
  ...
  try {
    ...
    const result = await groq.invoke([{ role: "user", content: analysisPrompt }]);
    ...
    try {
      ...
      if (jsonMatch) {
        try {
          ...
          console.log("✅ [DEBUG] Successfully extracted and parsed JSON:", analysis);
        } catch (extractError) {
          console.error("❌ [DEBUG] Even extracted JSON failed to parse:", extractError.message);
...
async function generateRecommendations(state) {
  ...
  console.log("💡 [DEBUG] Config check - Gemini key exists:", !!config.geminiApiKey);
...
Respond in a conversational, helpful manner in English. Keep your response comprehensive but easy to understand.
  ...
  try {
    ...
    console.log("💡 [DEBUG] Gemini response preview:", recommendation.substring(0, 200) + "...");
...
async function formatResponse(state) {
  ...
  const mainRecommendation = recommendations.length > 0 ? recommendations[0] : "I'm here to help with your farming questions!";
  ...
  if (safeContext.weather) {
    if (safeContext.weather.temp > 35) {
      additionalTips.push("🌡️ High temperature alert: Consider evening watering and shade protection for sensitive crops.");
    }
    if (safeContext.weather.humidity > 80) {
      additionalTips.push("💧 High humidity: Monitor for fungal diseases and ensure good air circulation.");
    }
    if (safeContext.weather.rain > 0) {
      additionalTips.push("☔ Rain detected: Adjust irrigation schedule accordingly.");
    }
  }
  
  if (safeContext.crop) {
    additionalTips.push(`🌱 For ${safeContext.crop}: Check our crop-specific guides for detailed care instructions.`);
  }
  
  if (safeContext.soilAnalysis) {
    additionalTips.push("🌍 Based on your soil analysis, consider the soil health recommendations provided.");
  }
...
function generateFallbackResponse(query, safeContext) {
  const responses = {
    ...
    market: "For market information, research local wholesale prices, connect with farmer producer organizations (FPOs), consider value addition, and explore direct marketing opportunities.",
    
    default: "I'm here to help with all your farming questions! You can ask about crop selection, pest management, soil health, weather planning, market prices, or any other agricultural topic."
  };
  
  const queryLower = query.toLowerCase();
  
  if (queryLower.includes('crop') || queryLower.includes('plant') || queryLower.includes('grow')) {
  ...
  } else if (queryLower.includes('soil') || queryLower.includes('fertilizer') || queryLower.includes('manure')) {
    return responses.soil;
  } else if (queryLower.includes('weather') || queryLower.includes('rain') || queryLower.includes('temperature')) {
    return responses.weather;
  } else if (queryLower.includes('market') || queryLower.includes('price') || queryLower.includes('sell')) {
...
function createFarmerAssistantWorkflow() {
  ...
  try {
    ...
    console.log("✅ [DEBUG] Workflow created successfully with channels approach");
    return workflow.compile();
    
  } catch (error1) {
    console.log("⚠️ [DEBUG] Channels approach failed, trying direct AgentState approach");
    ...
    try {
      ...
      console.log("✅ [DEBUG] Workflow created successfully with direct approach");
      return workflow.compile();
...
export async function executeFarmerAssistantPipeline(messages, context = {}) {
  ...
  if (config.groqApiKey) {
    console.log("🚀 [DEBUG] - GROQ key starts with:", config.groqApiKey.substring(0, 10) + "...");
  }
  if (config.geminiApiKey) {
    console.log("🚀 [DEBUG] - Gemini key starts with:", config.geminiApiKey.substring(0, 10) + "...");
  }
  ...
  try {
    ...
    console.log("🚀 [DEBUG] Initial state created with keys:", Object.keys(initialState));
    ...
    return {
      replies: [{
        role: "assistant",
        content: result.finalResponse || "I'm here to help with your farming questions!"
      }],
