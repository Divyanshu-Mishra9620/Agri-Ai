...
export async function getMarketTrends(req, res, next) {
  try {
    ...
    if (crop) {
      ...
      mockTrends.cropPrice = mockTrends.prices[crop.toLowerCase()] || "Price not available";
    }
  ...
  try {
    ...
    try {
      if (!config.geminiApiKey) {
        ...
        return res.json({
          success: true,
          summary: "The soil appears to be loamy with good organic content. pH seems neutral. Suitable for most crops.",
          recommendations: [
...
Keep the response concise but informative.
    ...
    res.json({
      success: true,
      summary: "Image analysis is temporarily unavailable. Please ensure good soil drainage, add organic matter, and test pH levels for optimal crop growth.",
      fallback: true
    });
  }
}