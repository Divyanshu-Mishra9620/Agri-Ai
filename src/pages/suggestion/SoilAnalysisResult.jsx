...
import { ChevronDown, ChevronUp, Leaf, AlertTriangle, CheckCircle, Eye, Droplets, Bug, TrendingUp } from 'lucide-react';
=======
import React, { useState, useMemo } from "react";
import {
  ChevronDown,
...
<<<<<<< HEAD
    ...
    const ratingMatch = content.match(/Overall\s+Health\s+Rating:?\*{0,2}\s*(\d+)\s*\/\s*10/i);
    const rating = ratingMatch ? parseInt(ratingMatch[1]) : null;
    
    // Extract main sections
    const soilConditionMatch = content.match(/\*{0,2}1\.\s*Soil\s+Condition\s+Assessment:?\*{0,2}(.*?)(?=\*{0,2}2\.|$)/si);
    const nutrientMatch = content.match(/\*{0,2}2\.\s*Nutrient\s+Deficiency\s+Signs?:?\*{0,2}(.*?)(?=\*{0,2}3\.|$)/si);
    const pestMatch = content.match(/\*{0,2}3\.\s*Pest\s+or\s+Disease\s+Indicators?:?\*{0,2}(.*?)(?=\*{0,2}4\.|$)/si);
    const recommendationsMatch = content.match(/\*{0,2}4\.\s*Specific\s+Recommendations?.*?\*{0,2}(.*?)(?=\*{0,2}5\.|$)/si);
    ...
    const generateVisualSummary = (soilText) => {
      ...
      if (lowerText.includes('dark') || lowerText.includes('healthy') || lowerText.includes('good')) {
        return 'The soil appears to be dark and healthy';
      } else if (lowerText.includes('light') || lowerText.includes('poor') || lowerText.includes('dry')) {
    ...
    const generateNutrientSummary = (nutrientText) => {
      ...
      if (lowerText.includes('no signs') || lowerText.includes('no clear') || lowerText.includes('adequate')) {
        return 'No clear signs of major nutrient deficiency';
      } else if (lowerText.includes('deficiency') || lowerText.includes('lacking') || lowerText.includes('low')) {
    ...
    const generatePestSummary = (pestText) => {
      ...
      if (lowerText.includes('no obvious') || lowerText.includes('no signs') || lowerText.includes('no pest') || lowerText.includes('healthy')) {
        return 'No obvious pest or disease symptoms';
      } else if (lowerText.includes('pest') || lowerText.includes('disease') || lowerText.includes('damage')) {
        return 'Potential pest or disease issues detected';
      }
      return 'Pest and disease assessment completed';
    };

    const soilCondition = soilConditionMatch ? soilConditionMatch[1].trim() : 'No soil condition data available';
    const nutrientDeficiency = nutrientMatch ? nutrientMatch[1].trim() : 'No nutrient analysis available';
    const pestDisease = pestMatch ? pestMatch[1].trim() : 'No pest/disease data available';
=======
...
>>>>>>> 94d473ce04d6ac32fbdd4070ec6afde3e0326c9e
      ...
      if (trimmed.match(/^\*{2,3}.*?:?\*{0,2}$/) || trimmed.match(/^\d+\.\s/)) {
        ...
        currentRec = {
<<<<<<< HEAD
          title: trimmed.replace(/\*{2,3}/g, '').replace(/^(\d+\.\s)?/, '').replace(':*', '').trim(),
          content: []
        };
...
<<<<<<< HEAD
          <h3 className="text-xl font-semibold text-gray-800">Soil Analysis Results</h3>
        </div>
        
        <div className={`flex items-center gap-2 px-4 py-2 rounded-full ${getRatingColor(analysis.rating)}`}>
...
<<<<<<< HEAD
              <span className="font-semibold text-gray-800">Soil Condition Assessment</span>
            </div>
            {expandedSections.soil ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
=======
...
<<<<<<< HEAD
              <span className="font-semibold text-gray-800">Nutrient Deficiency Analysis</span>
            </div>
            {expandedSections.nutrients ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
=======
              <span className="font-semibold text-gray-800">
...
<<<<<<< HEAD
              <span className="font-semibold text-gray-800">Pest & Disease Indicators</span>
            </div>
            {expandedSections.pests ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
=======
              <span className="font-semibold text-gray-800">
...
<<<<<<< HEAD
            <h4 className="text-lg font-semibold text-emerald-800">Action Recommendations</h4>
          </div>
          
          <div className="grid gap-4">
            {recommendations.map((rec, index) => (
              <div key={index} className="bg-white rounded-lg p-4 shadow-sm border border-emerald-200">
=======
...
<<<<<<< HEAD
                    <p key={idx} className="leading-relaxed whitespace-pre-line">{item}</p>
=======
                    <p
                      key={idx}
            ...
            <div className="flex items-center gap-2">
              <div className="w-32 bg-gray-200 rounded-full h-2">
                <div 
                  ...
                  style={{ width: `${Math.min(Math.max(soilResult.confidence, 0), 100)}%` }}
                ></div>
              </div>
              <span className="text-sm font-semibold text-gray-600">{soilResult.confidence}%</span>
=======
